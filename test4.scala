// =================================================================================================
// PATCH FOR CAPS IN TABLE/COLUMN NAMES (Mapping / Conditions)  ✅ FIXES "No join path from Unit to UNIT"
// -------------------------------------------------------------------------------------------------
// What was happening:
//   - Your TableGraph has tables like "Unit", but mapping/conditions sometimes use "UNIT" (caps).
//   - WideBuilder was treating them as different tables => BFS can't find join path.
//   - Resolver was also case-sensitive for columns like UNIT.Trailer_License_State_010101.
//
// Fix implemented below (drop-in updates):
//   1) WideBuilder canonicalizes table names case-insensitively against graph.tables keys.
//   2) RuleDependency stays as-is (it can extract "UNIT.Col"), but WideBuilder converts to real table names.
//   3) DestTableBuilder resolver matches column names case-insensitively, including dotted names.
//   4) Dependency extraction for Condition table.col still works even if caps; canonicalization handles it.
//
// NOTE: This is the FULL one-cell script including all prior features + CAPS FIX.
// =================================================================================================
...
// ------------------------------
// Dependency extraction (Condition can contain UNIT.Col in CAPS)
// ------------------------------
  ...
  def requiredTableColsForDest(mappingDf: DataFrame, destTable: String): Map[String, Set[String]] = {
    ...
    rows.foreach { r =>
      ...
      tableColRx.findAllMatchIn(cond).foreach { m =>
        add(m.group(1), m.group(2)) // may be "UNIT", will be canonicalized later in WideBuilder
      }
    }
...
// ------------------------------
// Wide DF Builder (CAPS FIX: canonicalize table names case-insensitively)
// ------------------------------
object WideBuilder {

  // Canonicalize table name against graph.tables keys (case-insensitive)
  private def canonicalTableName(graph: TableGraph, t: String): String = {
    val hit = graph.tables.keys.find(_.equalsIgnoreCase(t))
    hit.getOrElse(throw new IllegalArgumentException(s"Unknown table '$t' (case-insensitive lookup failed). Known: ${graph.tables.keys.toSeq.sorted.mkString(", ")}"))
  }
  ...
  private def selectWithAliases(tableName: String, df: DataFrame, cols: Set[String], keepKeys: Seq[String], keyPrefix: Option[String]): DataFrame = {
    ...
    // IMPORTANT: data columns are aliased as literal "Table.Column" using the canonical table name
  ...
  def buildWideDf(graph: TableGraph, mappingDf: DataFrame, destTable: String, baseTable: String): DataFrame = {
    ...
    // Canonicalize base table (case-insensitive)
    val base = canonicalTableName(graph, baseTable)

    // Canonicalize required tables (case-insensitive)
    ...
    val neededTables = (req.keySet + base).toSeq.distinct

    val baseNode = graph.tables.getOrElse(base, throw new IllegalArgumentException(s"Missing base table: $base"))
...
// ------------------------------
// DestTableBuilder (CAPS FIX: resolver matches columns case-insensitively)
// ------------------------------
object DestTableBuilder {
  ...
  // - resolves exact dotted names ignoring case ("UNIT.X" -> "Unit.X")
  // - resolves unqualified tails ignoring case (X -> unique *.X), prefers SourceTableName when ambiguous
  private def resolver(wideDf: DataFrame, defaultTable: Option[String]): String => Column = {
    // Build lookup maps once per resolver creation
    val cols = wideDf.columns
    val lowerToActual: Map[String, String] =
      cols.groupBy(_.toLowerCase).map { case (k, vs) => k -> vs.head } // deterministic enough
    ...
    (name0: String) => {
      ...
      def litCol(actual: String): Column = col(s"`$actual`") // dot-safe

      // 1) exact match (case-insensitive)
      lowerToActual.get(nameL) match {
        case Some(actual) => litCol(actual)
        case None =>
          // 2) tail match: if wide has plain column name
          lowerToActual.get(tailL) match {
            case Some(actualTail) => litCol(actualTail)
            case None =>
              // 3) dotted tail matches
              ...
              dottedMatches.length match {
                ...
                case 0 =>
                  throw new IllegalArgumentException(
                    s"Cannot resolve column '$name0' (case-insensitive). Example columns: ${cols.take(50).mkString(", ")}"
                  )

                case _ =>
                  // Prefer default table when ambiguous
                  val preferred = defaultTable.flatMap { t =>
                    val candL = (t + "." + tail).toLowerCase
                    lowerToActual.get(candL)
                  }
                  preferred match {
                    case Some(actual) => litCol(actual)
                    case None =>
...
// ------------------------------
// Public helper: build destination output
// ------------------------------
def buildDestTable(destTableName: String, baseTable: String, graph: TableGraph, mappingDf: DataFrame): DataFrame = {
  val baseNode = graph.tables.getOrElse(graph.tables.keys.find(_.equalsIgnoreCase(baseTable)).getOrElse(baseTable),
    throw new IllegalArgumentException(s"Missing base table: $baseTable"))
...
// )
//
// // baseTable can be "UNIT" or "Unit" now (case-insensitive)
// val vehicleOut = buildDestTable("Vehicle", "UNIT", graph, mappingDf)
