// =================================================================================================
// FINAL ONE-CELL SCRIPT (ALL PREVIOUS FIXES + NEW: <DestTableName>_id)
// -------------------------------------------------------------------------------------------------
// NEW REQUIREMENT IMPLEMENTED (replaces previous request):
// ✅ For EACH destination table output DF, add ONE column named: <DestinationTableName>_id
// ✅ This id is generated AFTER the destination DF is fully built (AFTER multi-source union)
// ✅ Id logic:
//    - Partition by Crash_Id (case-insensitive)
//    - row_number() per Crash_Id ordered by:
//        _BaseSourceTable + ALL PK columns across ALL involved source tables (string-coalesced)
//    - Result: for each crash_id, rows get 1..N (even if rows are from multiple sources)
//      Example: crash_id has 3 Person rows across 3 sources => Person_id = 1,2,3
// ✅ Column appended at the END of dataframe.
// -------------------------------------------------------------------------------------------------
// Keeps everything else:
// ✅ Returns Map[DestTableName -> DataFrame]
// ✅ Multi-source destination union ALL rows (keeps duplicates, never overwrites)
...
// ✅ Adds Id (sha2) without dropping duplicates + _BaseSourceTable marker
...
object DestTableBuilder {
  ...
  def buildForDestTable(destTableName: String, baseTable: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): (DataFrame, Seq[String]) = {
    ...
    val rows = mappingDf
      ...
      .collect()
      .toSeq
      // For each base-table pass, only rules from that base (plus NULL + <DEFAULT>)
      .filter { r =>
        val st = cleanOpt(r.getAs[Any]("SourceTableName"))
        val sc = cleanOpt(r.getAs[Any]("SourceColumnName"))
    ...
    val rulesByDestCol: Map[String, Seq[MapRule]] =
      rows.map { r =>
        MapRule(
          ...
          conversionType   = Try(r.getAs[Any]("ConversionType").toString.trim.toInt).getOrElse(1),
          conditionRaw     = cleanOpt(r.getAs[Any]("Condition")),
          priority         = Try(r.getAs[Any]("Priority").toString.trim.toInt).getOrElse(999999)
        )
      }
...
// =================================================================================================
// AUTOMATION + NEW: Add <DestTableName>_id at END (after all sources unioned)
// =================================================================================================
object MappingAutomation {
  ...
  // ----- NEW: Build <DestTableName>_id per crash using all PKs from all involved sources -----
  private def addDestTableIdAtEnd(
    ...
    // resolve crash id column case-insensitive
    ...
    // gather all pk logical names across involved sources, then map to actual columns in df (case-insensitive)
    ...
    // Ensure we have deterministic order even if pk not present: include what exists + base source marker if available
    val baseSourceActualOpt = lowerToActual.get("_basesourcetable") // from our pipeline
  ...
  def buildAllDestTablesAsMap(
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      // ✅ NEW: add <DestTableName>_id at end
      val finalDf = addDestTableIdAtEnd(
        destName = dest,
        df = unioned,
...
// )
//
// display(outMap("Person"))      // has Person_id (1..N per crash_id)
// display(outMap("Vehicle"))     // has Vehicle_id
// =================================================================================================
