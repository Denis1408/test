...
object MappingAutomation {
  ...
  // Build graph for needed tables (load DFs in-loop)
  ...
  // Add Id when dest uses >1 source table; Id from PKs across involved tables (hash)
  ...
  def buildAllDestTablesAsMap(
    ...
    pkByTable: Map[String, Seq[String]],
    joinEdges: Seq[JoinEdge],
    sourceTablePriority: Map[String, Int] = Map.empty, // base selection priority
    destTablePriority: Map[String, Int] = Map.empty,   // processing order priority
    ...
    // Build each dest df and return as Map
    val outPairs = destSorted.map { dest =>
      ...
      // plus SourceTableName distinct (helps base selection)
      ...
      // graph for needed tables (loads DFs)
      val graph = buildGraphForTables(spark, tableLoader, pkByTable, joinEdges, involvedTables)

      // choose base by sourceTablePriority (prefer mapping SourceTableName list)
      val baseCandidates =
...
// )
//
// // RESULT: ONE VALUE holding MULTIPLE DATAFRAMES
