// =================================================================================================
// PATCHED ONE-CELL SCRIPT (ONLY CHANGES YOU ASKED):
// -------------------------------------------------------------------------------------------------
// ✅ Dest table id column name: <DestTableName>Id   (NO underscore)
// ✅ Ability to EXCLUDE some destination tables from adding <DestTableName>Id
//    - new param: excludeDestIdFor: Set[String] (case-insensitive match)
//    - if dest in exclude list => do NOT add the DestTableNameId column
// -------------------------------------------------------------------------------------------------
// NOTE: Everything else remains identical to your currently working version.
// =================================================================================================
...
// =================================================================================================
// AUTOMATION + PATCH: <DestTableName>Id (no underscore) + exclude list
// =================================================================================================
object MappingAutomation {
  ...
  // NEW: Add <DestTableName>Id (no underscore) at END, unless excluded
  private def addDestTableIdAtEnd(
    ...
    val orderCols: Seq[Column] =
      (baseSourceActualOpt.toSeq.map(c => col(s"`$c`")) ++ allPkActual.map(c => coalesce(col(s"`$c`").cast("string"), lit(""))))
    ...
    val destIdCol = s"${destName}Id"   // ✅ no underscore
  ...
  def buildAllDestTablesAsMap(
    ...
    excludeDestIdFor: Set[String] = Set.empty,             // ✅ NEW PARAM
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      // ✅ add <DestName>Id at end (unless excluded)
...
//   excludeDestIdFor = Set("Address","SomeOtherDest")   // <- exclude tables from adding <DestName>Id
// )
//
// display(outMap("Person"))   // has PersonId
// display(outMap("Address"))  // NO AddressId
// =================================================================================================
