...
// =================================================================================================
// AUTOMATION (FIXED): NEVER TREATS DESTINATION TABLES AS SOURCE TABLES
// =================================================================================================
object MappingAutomation {
  ...
  // Case-insensitive canonicalization (ONLY for SOURCE tables)
  ...
  private def buildGraphForTables(
    ...
    sourceTablesNeeded: Set[String]  // already canonicalized source tables only
  ): TableGraph = {
    ...
    val canonEdges = joinEdges
      .filter(e => nodes.contains(e.leftTable) && nodes.contains(e.rightTable))

    TableGraph(nodes, canonEdges)
  }

  private def addIdIfNeeded(baseTable: String, involvedSourceTables: Set[String], graph: TableGraph, wideDf: DataFrame, mappedDf: DataFrame): DataFrame = {
    if (involvedSourceTables.size <= 1) return mappedDf
    ...
    val pkAll = involvedSourceTables.toSeq.sortBy(_.toLowerCase).flatMap(pkColsForTable).distinct
  ...
  def buildAllDestTablesAsMap(
    ...
    // Canonicalize joinEdges ONLY for SOURCE tables; drop any edges referencing non-source tables
    ...
    val outPairs = destSorted.map { dest =>
      ...
      // ✅ IMPORTANT FIX:
      // keep ONLY valid SOURCE tables (present in pkByTable). Destination tables like "Address" are dropped.
      ...
      if (involvedSourceTables.isEmpty) {
        throw new IllegalArgumentException(
          ...
          s"Check mapping SourceTableName / Condition tables. Known SOURCE tables: ${sourceUniverse.toSeq.sorted.mkString(", ")}"
        )
      }

      // choose base by sourceTablePriority
      val baseTable = chooseBaseTable(involvedSourceTables, sourceTablePriority)

      // build graph only with SOURCE tables
      ...
      // apply mapping -> base PK + mapped cols (+Text)
      val (mappedOnly, _) = DestTableBuilder.buildForDestTable(dest, graph.tables(baseCanon).pk, wide, mappingDf)

      // add Id if multi-source
      val finalDf = addIdIfNeeded(baseCanon, involvedSourceTables, graph, wide, mappedOnly)
