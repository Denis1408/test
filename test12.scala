// =================================================================================================
// FINAL ONE-CELL SCRIPT (PERMANENT FIX INCLUDED)
...
//    - buildAllDestTablesAsMap now accepts joinEdgesRaw: Seq[_]
...
// ✅ MULTI-SOURCE DEST FIX:
//      - For each destination, we process EACH involved source table as a base and UNION ALL.
//      - Same Crash_Id across sources is kept (no overwrite).
//      - Duplicates inside source are kept.
// ✅ Id generation FIX:
//      - Id is built ONLY from base PK columns that exist in mappedDf + baseTable + row_number.
//      - Never references "__k__OtherTable__Pk" columns (prevents UNRESOLVED_COLUMN).
// =================================================================================================
...
// =================================================================================================
// AUTOMATION (PERMANENT JoinEdge FIX INCLUDED)
// =================================================================================================
object MappingAutomation {
  ...
  // ✅ PERMANENT FIX: normalize ANY JoinEdge-like case class from other cells to THIS JoinEdge
  // Accepts:
  //   - this cell's JoinEdge
  //   - any Product with (leftTable, rightTable, leftKeys, rightKeys, [joinType])
  ...
  // MAIN (joinEdgesRaw is Seq[_] to avoid notebook type mismatch forever)
...
//   mappingDf = mappingDf,
//   pkByTable = pkByTable,
//   joinEdgesRaw = joinEdges,              // <-- can be from any cell/type now
