// =================================================================================================
// FINAL ONE-CELL SCRIPT (ALL FIXES + TYPE1 <FORMAT> + SAFE STRING PARSING + NEW: SourceTableName_id)
// -------------------------------------------------------------------------------------------------
// NEW REQUIREMENT IMPLEMENTED:
// ✅ For EACH destination table output DF, add columns named: <SourceTableName>_id (for each source table used by that dest)
// ✅ These ids are generated AFTER the destination DF is built (after multi-source union)
// ✅ Id logic:
//    - Partition by crash_id
//    - Create a GLOBAL row_number per crash_id across ALL rows (from ALL source tables) ordered by:
//        _BaseSourceTable + all PK columns across all involved source tables (string-coalesced)
//    - For each source table T: column T_id = globalRowNumber when _BaseSourceTable == T else NULL
// ✅ Example: for crash_id with 3 rows coming from 3 different source tables:
//      row ids become 1,2,3 (per crash_id) and the corresponding <SourceTable>_id column is populated.
// ✅ These <SourceTable>_id columns are appended at the END of the dataframe.
// -------------------------------------------------------------------------------------------------
// Also keeps everything from prior version:
// ✅ Multi-destination -> Map[DestTableName -> DataFrame]
// ✅ Multi-source destination: union ALL rows (keeps duplicates)
// ✅ No “overwrite/update” for same crash_id
// ✅ Type2: SourceValue NULL or <NULL> = wildcard, condition decides
// ✅ Conditions: IS NULL/IS NOT NULL/=NULL/!=NULL/AND/OR/&&/||/IN/NOT IN
// ✅ Type1: <VALUE> + <FORMAT> uses Condition as pattern and safe string parsing via try_to_timestamp(lit(pattern))
// ✅ DestValueDescription => DestColumnText
// ✅ Adds Id (sha2) without dropping duplicates
...
// ------------------------------
// DestTableBuilder (Type2 wildcard + Type1 <FORMAT> + SAFE string parsing)
// ------------------------------
object DestTableBuilder {
  ...
  // ✅ try_to_timestamp format must be Column => lit("pattern")
  ...
  private def buildExpr(
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      val condOk: Column =
        if (fmtRule) lit(true) // Condition is a pattern string for <FORMAT>
  ...
  def buildForDestTable(destTableName: String, baseTable: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): (DataFrame, Seq[String]) = {
    ...
    val rows = mappingDf
      ...
      .collect()
      .toSeq
      // IMPORTANT: For each "baseTable run" apply ONLY rules from that base table (plus NULL + <DEFAULT>)
      .filter { r =>
        val st = cleanOpt(r.getAs[Any]("SourceTableName"))
        val sc = cleanOpt(r.getAs[Any]("SourceColumnName"))
...
// =================================================================================================
// AUTOMATION + NEW: add <SourceTableName>_id columns at END (after all mapping for dest is done)
// =================================================================================================
object MappingAutomation {
  ...
  // Normalize joinEdgesRaw (can be Seq[JoinEdge] or tuples etc)
  ...
  // Add Id (sha2) WITHOUT dropping duplicates (keeps all rows per crash)
  ...
  // -------- NEW: add <SourceTableName>_id columns based on crash_id and PKs across ALL involved sources --------
  private def findColNameCI(df: DataFrame, name: String): Option[String] = {
    val m = df.columns.groupBy(_.toLowerCase).map { case (k, vs) => k -> vs.head }
    m.get(name.toLowerCase)
  }

  private def addSourceTableIdsAtEnd(
    destDf: DataFrame,
    involvedSourceTables: Seq[String],          // canonical names
    pkByTable: Map[String, Seq[String]],
    crashIdName: String = "Crash_Id",           // expected logical name
    baseSourceCol: String = "_BaseSourceTable"  // must exist
  ): DataFrame = {

    val crashColActual = findColNameCI(destDf, crashIdName)
      .getOrElse(throw new IllegalArgumentException(
        s"Destination DF missing crash id column (case-insensitive) '$crashIdName'. Existing: ${destDf.columns.sorted.mkString(", ")}"
      ))

    if (!destDf.columns.exists(_.equalsIgnoreCase(baseSourceCol))) {
      throw new IllegalArgumentException(s"Destination DF missing internal column '$baseSourceCol' (needed to build <SourceTableName>_id).")
    }
    val baseSourceActual = findColNameCI(destDf, baseSourceCol).get

    // Collect all PK columns across all involved source tables (unique list)
    val allPkLogical: Seq[String] =
      involvedSourceTables.flatMap(t => pkByTable.getOrElse(t, Nil)).distinct

    // Keep only PK columns that exist in the union DF (case-insensitive)
    val lowerToActual = destDf.columns.groupBy(_.toLowerCase).map { case (k, vs) => k -> vs.head }
    val pkActualCols: Seq[String] =
      allPkLogical.flatMap(pk => lowerToActual.get(pk.toLowerCase)).distinct

    // Window ordering: stable order by _BaseSourceTable + all PK columns (string-coalesced)
    val orderCols: Seq[Column] =
      (Seq(col(s"`$baseSourceActual`")) ++ pkActualCols.map(c => coalesce(col(s"`$c`").cast("string"), lit(""))))

    val w = Window.partitionBy(col(s"`$crashColActual`")).orderBy(orderCols: _*)
    val globalIdColName = "__global_src_id__"

    var df = destDf.withColumn(globalIdColName, row_number().over(w))

    // Build id columns: <SourceTable>_id, set only for rows coming from that base source table
    val newIdCols: Seq[String] = involvedSourceTables.distinct.map(t => s"${t}_id")

    involvedSourceTables.distinct.foreach { t =>
      val colName = s"${t}_id"
      df = df.withColumn(
        colName,
        when(lower(col(s"`$baseSourceActual`")) === lit(t.toLowerCase), col(globalIdColName)).otherwise(lit(null).cast("int"))
      )
    }

    // Reorder so new id columns are appended at the end
    val baseCols = df.columns.filterNot(c => newIdCols.contains(c)).filterNot(_ == globalIdColName)
    df.select((baseCols.map(c => col(s"`$c`")) ++ newIdCols.map(c => col(s"`$c`"))): _*)
  }
  ...
  def buildAllDestTablesAsMap(
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      // base processing order (sourceTablePriority)
      ...
      // Build per-base mapped df and UNION ALL (keeps duplicates, no overwrite for same crash_id)
      ...
      // -------- NEW: add <SourceTableName>_id columns at the end (after union) --------
      val withSourceIds = addSourceTableIdsAtEnd(
        destDf = unioned,
        involvedSourceTables = involvedSourceTables,
        pkByTable = pkByTable,
        crashIdName = "Crash_Id",
        baseSourceCol = "_BaseSourceTable"
      )
