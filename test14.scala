// =================================================================================================
// FINAL ONE-CELL SCRIPT (ALL FIXES INCLUDED)
// Mapping-driven load from MANY source tables -> MANY destination tables
// Returns: Map[DestTableName -> DataFrame]
// -------------------------------------------------------------------------------------------------
// ✅ PERMANENT JoinEdge mismatch fix: joinEdgesRaw: Seq[_], normalize Product -> JoinEdge
// ✅ MULTI-SOURCE DEST: process EACH involved source table as base, UNION ALL rows
// ✅ NO CROSS-ENRICH: when base is X, apply ONLY rules with SourceTableName==X
//    plus global rules (SourceTableName NULL) plus <DEFAULT> rules
...
//      - SourceValue == NULL     => wildcard (any value)  ✅ NEW FIX
...
// ✅ You MUST set tableLoader if your tables are not directly spark.table("Unit")
// =================================================================================================
...
// ------------------------------
// DestTableBuilder (NO CROSS-ENRICH + TYPE2 NULL WILDCARD FIX HERE)
// ------------------------------
object DestTableBuilder {
  ...
  private def buildExpr(
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      // ✅ FIX: Type 2 SourceValue NULL => wildcard (any value), rely on condition
      val srcOk: Column = (r.sourceColumn, r.sourceValueRaw) match {
        ...
        case (Some(_), None) if r.conversionType == 2 => lit(true)          // ✅ NEW: NULL wildcard for type2
        case (Some(sc), None) => resRule(sc).isNull                          // keep old behavior for type1
...
//   mappingDf = mappingDf,
//   pkByTable = pkByTable,
//   joinEdgesRaw = joinEdges,                 // any JoinEdge type ok
