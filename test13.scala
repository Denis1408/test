// =================================================================================================
// FINAL ONE-CELL SCRIPT (WITH MULTI-SOURCE NO-CROSS-ENRICH FIX + PERMANENT JoinEdge FIX)
// Mapping-driven load from MANY source tables -> MANY destination tables
// Returns: Map[DestTableName -> DataFrame]
// -------------------------------------------------------------------------------------------------
// ✅ PERMANENT FIX for JoinEdge type mismatch across notebook cells:
//    - buildAllDestTablesAsMap accepts joinEdgesRaw: Seq[_]
//    - internally normalizes ANY "JoinEdge-like" case class (Product) into this cell's JoinEdge
//
// ✅ MULTI-SOURCE DEST FIX (NO CROSS-ENRICH / NO OVERWRITE):
//    - For a destination that depends on multiple source tables:
//        * we process EACH involved source table as a base and UNION ALL rows
//        * BUT when base is X, we ONLY APPLY mapping rules whose SourceTableName == X
//          (plus "global" rules with SourceTableName NULL, plus <DEFAULT> rules)
//    - This prevents last base run from populating earlier base rows via joined columns.
//
...
// ✅ Keeps duplicates; adds Id from base PK + baseTable + row_number (no dropDuplicates).
// ✅ You MUST set tableLoader if your source tables are not in current schema (spark.table("Unit")).
// =================================================================================================
...
// ------------------------------
// DestTableBuilder (NO CROSS-ENRICH FIX HERE)
// ------------------------------
object DestTableBuilder {
  ...
  // ✅ UPDATED SIGNATURE: accepts baseTable and filters rules to avoid cross-enrich overwrite
  def buildForDestTable(destTableName: String, baseTable: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): (DataFrame, Seq[String]) = {
    ...
    // Collect rows for destination and FILTER:
    // - keep rules where SourceTableName == baseTable
    // - keep rules where SourceTableName is NULL (global)
    // - keep <DEFAULT> rules (global)  (SourceColumnName NULL + SourceValue <DEFAULT>)
    val rows = mappingDf
...
// =================================================================================================
// AUTOMATION (PERMANENT JoinEdge FIX + MULTI-SOURCE UNION ALL + NO CROSS-ENRICH)
// =================================================================================================
object MappingAutomation {
  ...
  // normalize ANY JoinEdge-like case class from other cells to THIS JoinEdge
  ...
  // Id uses ONLY base PK columns present in mappedDf, preserves duplicates
  private def addIdWithoutDroppingDuplicates(
    ...
    withRn
      ...
      .drop(rnColName)
      .withColumn("_BaseSourceTable", lit(baseTable)) // drop if you don't need it
  }
  ...
  def buildAllDestTablesAsMap(
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      // keep ONLY SOURCE tables that exist in pkByTable
      ...
      // Multi-source: process each involved source table as base then UNION ALL
      ...
      val perBase: Seq[DataFrame] = baseTablesOrdered.map { baseTable =>
        ...
        // ✅ IMPORTANT: pass baseCanon so builder filters rules to SourceTableName==baseCanon
...
// USAGE
// -------------------------------------------------------------------------------------------------
// 1) Schema loader example:
// val tableLoader: String => DataFrame = (t: String) => spark.table(s"catalog.schema.$t")
//
// 2) In-memory DF map example:
// val sourceDfs = Map("Unit"->unitDf, "Summary"->summaryDf, "Person_Injured"->personDf)
// val tableLoader: String => DataFrame = (t: String) =>
//   sourceDfs.collectFirst{ case (k, df) if k.equalsIgnoreCase(t) => df }
//     .getOrElse(throw new IllegalArgumentException(s"Missing DF for source table: $t. Provided: ${sourceDfs.keys.mkString(", ")}"))
//
// val outMap = MappingAutomation.buildAllDestTablesAsMap(
//   spark = spark,
//   mappingDf = mappingDf,
//   pkByTable = pkByTable,
//   joinEdgesRaw = joinEdges,                 // any cell/type ok
