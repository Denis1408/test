...
object ConditionParser {
  ...
  private val oneCharOps = Set("=", "<", "zzz">", "!")
...
object MappingAutomation {
  ...
  // ✅ FIXED Id: uses ONLY base PK columns present in mappedDf
  ...
  def buildAllDestTablesAsMap(
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      // process each involved source table as base (priority order), then UNION ALL
...
// =================================================================================================
// USAGE (you must set loader if tables not in current schema)
// -------------------------------------------------------------------------------------------------
// val loader: String => DataFrame = (t: String) => spark.table(s"catalog.schema.$t")
