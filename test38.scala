...
object DestTableBuilder {
  ...
  private def buildExpr(
    ...
    val (finalVal, _) = rules.foldLeft((lit(null).cast("string"), lit(false))) { case ((acc, matched), r) =>
      ...
      val srcOk: Column = (r.sourceColumn, r.sourceValueRaw) match {
        ...
        case (Some(sc), None) =>
          ...
          if (hasCond) {
            ...
            val nullish = c.isNull || (cStr === lit("NULL")) || (cStr === lit(""))
            val nanish = isnan(c.cast("double"))
            nullish || coalesce(nanish, lit(false))
          }
      ...
      val effectiveCond = if (whenOnlyIfAccNull) (!matched && ruleMatch) else ruleMatch
      val nextAcc = when(effectiveCond, destVal).otherwise(acc)
      val nextMatched = matched || effectiveCond
      (nextAcc, nextMatched)
    }
    finalVal
  }
  ...
  def buildForDestTable(
    ...
    rulesByDestCol.toSeq.sortBy(_._1).foreach { case (destCol, allRulesSorted) =>
      ...
      val normalVal = buildExpr(destTableName, destCol, normalRules, out, whenOnlyIfAccNull = true, _.destValueRaw, passThroughOnValueToken = true)
      ...
      val normalTxt = buildExpr(destTableName, destCol, normalRules, out, whenOnlyIfAccNull = true, _.destValueDescRaw, passThroughOnValueToken = false)
