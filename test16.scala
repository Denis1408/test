// =================================================================================================
// FINAL ONE-CELL SCRIPT (ALL FIXES + TYPE1 <FORMAT> FIX FOR TEXT PASS)
// -------------------------------------------------------------------------------------------------
// ✅ Type1 <VALUE> + DestValue <FORMAT> => format by Condition pattern (yyyy-MM-dd, HHmm, etc)
// ✅ FIX: When rule is <FORMAT>, Condition is NOT parsed as boolean even when building Text columns
// =================================================================================================
...
// ------------------------------
// DestTableBuilder (FIXED: format rules skip boolean parsing always)
// ------------------------------
object DestTableBuilder {
  ...
  private def buildExpr(
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      // ✅ detect format-rule from the RULE itself (not from valueSelector)
      val fmtRule: Boolean =
        r.conversionType == 1 &&
        isValueToken(r.sourceValueRaw) &&
        isFormatToken(r.destValueRaw)
      ...
      // ✅ if it's a format rule, Condition is a pattern string, NOT boolean => skip parsing always
      val condOk: Column =
        if (fmtRule) lit(true)
      ...
      val destVal: Column = {
        // Apply formatting ONLY when building the VALUE column (raw == <FORMAT>)
        if (fmtRule && isFormatToken(raw)) {
          ...
          date_format(resRule(srcCol).cast("timestamp"), fmt)
