// =================================================================================================
// FINAL ONE-CELL SCRIPT — Mapping-driven load from MANY source tables -> MANY destination tables
// Returns: Map[DestTableName -> DataFrame]
// -------------------------------------------------------------------------------------------------
// ✅ Loads source tables via tableLoader (you MUST set schema or DF-map if not in current schema)
// ✅ Destination table names are NEVER treated as source tables (no PK required for dest tables)
// ✅ Case-insensitive table + column resolution (UNIT vs Unit, etc.)
// ✅ Condition parser supports: IS NULL / IS NOT NULL / = NULL / != NULL / IN / NOT IN / AND / OR / && / ||
// ✅ Type 1 + Type 2 mappings
// ✅ Type 2: SourceValue == "<NULL>" => wildcard (matches any source value including null)
// ✅ <VALUE> pass-through, <DEFAULT> fallback rule when no other rule matched for a dest column
// ✅ DestValueDescription -> creates "<DestColumnName>Text" column
// ✅ IMPORTANT FIX for multi-source destination:
//      - If a destination depends on >1 source tables, we DO NOT pick one base.
//      - We build results for EACH involved source table as a base and UNION ALL.
//      - Same Crash_Id from different sources is kept (no overwrite).
// ✅ Adds unique Id for multi-source outputs WITHOUT dropping duplicates (uses row_number partitioned by base PK)
// =================================================================================================
...
// ------------------------------
// Wide DF Builder (joins along graph; keeps PKs with prefixes for non-base tables)
// ------------------------------
object WideBuilder {
...
// ------------------------------
// DestTableBuilder (mapping rules -> dest columns + Text columns + DEFAULT fallback)
// ------------------------------
object DestTableBuilder {
  ...
  // Case-insensitive + ambiguity-safe resolver
  ...
  def buildForDestTable(destTableName: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): (DataFrame, Seq[String]) = {
    ...
    rulesByDestCol.toSeq.sortBy(_._1).foreach { case (destCol, allRulesSorted) =>
      ...
      // DEFAULT rule: SourceColumnName is NULL AND SourceValue == "<DEFAULT>"
...
object MappingAutomation {
  ...
  // Default loader (override if your tables live in schema/catalog or are in-memory DFs)
  // Example schema loader: (t: String) => spark.table(s"prod.raw.$t")
  ...
  // Canonicalize SOURCE table name (case-insensitive)
  ...
  // Build graph only from SOURCE tables
  ...
  // Add unique Id WITHOUT dropping duplicates.
  // If same base PK repeats, row_number makes Id unique and preserves all rows.
  private def addIdWithoutDroppingDuplicates(
    baseTable: String,
    involvedSourceTables: Set[String],
    graph: TableGraph,
    wideDf: DataFrame,
    mappedDf: DataFrame
  ): DataFrame = {
    ...
    val pkAll: Seq[String] =
      involvedSourceTables.toSeq.sortBy(_.toLowerCase).flatMap(pkColsForTable).distinct
    ...
    val idParts: Seq[Column] =
      (pkAll.flatMap { c =>
        if (wideDf.columns.contains(c)) Some(coalesce(col(s"`$c`").cast("string"), lit(""))) else None
      } :+
        lit(baseTable) :+
    ...
    withRn
      ...
      .drop(rnColName)
      .withColumn("_BaseSourceTable", lit(baseTable)) // keep for auditing/debugging (drop later if you want)
  }
  ...
  def buildAllDestTablesAsMap(
    ...
    // Canonicalize joinEdges ONLY for SOURCE tables (drop edges referencing non-source)
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      // Source tables referenced by mapping + conditions
      val neededTablesRaw = RuleDependency.requiredTableColsForDest(mappingDf, dest).keySet

      // Source tables listed directly in mapping
      ...
      // ✅ IMPORTANT: keep ONLY SOURCE tables that exist in pkByTable
      ...
      if (involvedSourceTables.isEmpty) {
        throw new IllegalArgumentException(
          s"DestTable='$dest' has no valid SOURCE tables in mapping/conditions. " +
          s"Known SOURCE tables: ${sourceUniverse.toSeq.sorted.mkString(", ")}"
        )
      }

      // Graph once per destination
      val graph = buildGraphForTables(tableLoader, pkByTable, joinEdges, involvedSourceTables)

      // ✅ FIX for multi-source destination:
      // Process EACH involved source table as base, then UNION ALL.
      ...
      val perBase: Seq[DataFrame] = baseTablesOrdered.map { baseTable =>
        ...
        addIdWithoutDroppingDuplicates(baseCanon, involvedSourceTables, graph, wide, mappedOnly)
      }
...
// USAGE
// -------------------------------------------------------------------------------------------------
// 1) If tables are in a schema/catalog, set loader like:
// val loader: String => DataFrame = (t: String) => spark.table(s"your_catalog.your_schema.$t")
//
// 2) Or if you have in-memory DFs:
// val sourceDfs = Map("Unit"->unitDf,"Summary"->summaryDf, ...)
// val loader: String => DataFrame = (t: String) => sourceDfs.collectFirst{case(k,df) if k.equalsIgnoreCase(t)=>df}
//   .getOrElse(throw new IllegalArgumentException(s"Missing DF for $t"))
//
