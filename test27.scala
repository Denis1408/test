// =================================================================================================
// ONE-CELL FINAL SCRIPT
// - Type2 correlated COUNT in condition: Unit.COUNT(Crash_Id) == 1 (correlated by current Crash_Id)
// - excludeDestIdFor: exclude certain destination tables from DestNameId column creation
// - renameRulesByDest: last-step renames (only if old exists and new doesn't exist)
// =================================================================================================
...
// =================================================================================================
// WIDE BUILDER + correlated COUNT columns
// =================================================================================================
object WideBuilder {
  ...
  // case-insensitive resolve in wide DF
  ...
  // ✅ Attach correlated COUNT aggregates as wide columns
  ...
  def buildWideDf(
    ...
    neededTables.filterNot(_ == base).foreach { target =>
      ...
      pathEdges.foreach { e =>
        ...
        if (!joined.contains(nextTable)) {
          ...
          wide = if (keepAllPk) joinedDf else joinedDf.drop(rk.map(rKey => s"${pfx}${rKey}"): _*)
          joined += nextTable
        }
        curTable = nextTable
      }
    ...
    // ✅ add correlated counts used by conditions
...
// =================================================================================================
// DEST TABLE BUILDER (COUNT rewrite before parse)
// =================================================================================================
  ...
  private def buildExpr(
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      val srcOk: Column = (r.sourceColumn, r.sourceValueRaw) match {
        ...
        // Type2: NULL / <NULL> means wildcard any value; condition decides
      ...
      val condOk: Column = r.conditionRaw match {
        ...
        case Some(c0) =>
          ...
          val rewritten = RuleDependency.rewriteCountFunctions(cleaned) // ✅ COUNT rewrite
...
// =================================================================================================
// AUTOMATION (RESTORED PARAMETERS)
// =================================================================================================
object MappingAutomation {
  ...
  // Stable Id per base table WITHOUT dropping duplicates
  ...
  // Add DestTableNameId (WITHOUT underscore) per Crash_Id partition
  ...
  // MAIN API (same signature your call expects)
...
// =================================================================================================
// USAGE EXAMPLE (your call now compiles again)
// -------------------------------------------------------------------------------------------------
