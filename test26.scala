// =================================================================================================
// ONE-CELL SCRIPT: Mapping Engine + NEW Type2 Correlated COUNT condition
// -------------------------------------------------------------------------------------------------
// NEW FEATURE (requested):
// ✅ Support mapping Condition like:  Unit.COUNT(Crash_Id) == 1
// Meaning (CORRELATED):
//   For the CURRENT output row (with some Crash_Id value),
//   count rows in SOURCE table Unit where Unit.Crash_Id == <current row Crash_Id>,
//   then compare that count to 1.
//
// Implementation:
// 1) Detect any "<Table>.COUNT(<Col>)" inside Condition (case-insensitive COUNT).
// 2) Build an aggregate DF per (Table, Col):  table.groupBy(Col).count()
// 3) Join that aggregate into the wide DF using CURRENT ROW value of <Col>
// 4) Rewrite condition: Unit.COUNT(Crash_Id) -> __agg__Unit__count__Crash_Id
// 5) Parse rewritten condition using the existing ConditionParser.
//
// Notes:
// - Works with COUNT / Count / count
// - Works with CAPS names in conditions (UNIT.COUNT(CRASH_ID))
// - Still supports: AND/OR/&&/||, IN, IS NULL, =NULL, !=NULL, etc.
// - Type2 rules: SourceValue NULL or <NULL> means wildcard "any source value", condition decides.
// =================================================================================================
...
// ------------------------------ Condition Parser ------------------------------
// Supports boolean logic, comparisons, IN/NOT IN, IS NULL/IS NOT NULL and =NULL/!=NULL
...
// =================================================================================================
// RULE DEPENDENCY (UPDATED): extracts both normal column deps and COUNT deps
// =================================================================================================
object RuleDependency {
  private val tableColRx: Regex =
    """([A-Za-z_][A-Za-z0-9_]*)\s*\.\s*([A-Za-z_][A-Za-z0-9_]*)""".r

  // ✅ COUNT detector (case-insensitive)
  ...
  // Returns: (columnsNeeded, countNeeded)
  def requiredDepsForDest(mappingDf: DataFrame, destTable: String): (Map[String, Set[String]], Map[String, Set[String]]) = {
    ...
    rows.foreach { r =>
      ...
      // normal Table.Column refs (skip ".COUNT" token)
  ...
  // rewrite "Unit.COUNT(Crash_Id)" -> "__agg__Unit__count__Crash_Id"
...
// =================================================================================================
// WIDE BUILDER (UPDATED): after joins, attach correlated COUNT aggregates as columns
// =================================================================================================
object WideBuilder {
  ...
  // Resolve column in wide DF case-insensitively (supports "Crash_Id" and "Unit.Crash_Id" etc)
  private def resolveWideColCI(wideDf: DataFrame, name0: String): Column = {
  ...
  // ✅ Add correlated COUNT aggregates: groupBy(Col).count(), join on current row Col value
  private def attachCountAggs(graph: TableGraph, wideDf: DataFrame, countAggsRaw: Map[String, Set[String]]): DataFrame = {
    ...
    countAggs.foldLeft(wideDf) { case (curWide, (tableName, colsToCount)) =>
      ...
      colsToCount.foldLeft(curWide) { case (wide2, colToCount) =>
        // resolve actual column name in that source DF
  ...
  // Build wide DF for one base table, then attach count aggregates
  def buildWideDf(
    graph: TableGraph,
    mappingDf: DataFrame,
    destTable: String,
...
// =================================================================================================
// DEST TABLE BUILDER (UPDATED): rewrites COUNT() in Condition before parsing
// =================================================================================================
object DestTableBuilder {
  ...
  // case-insensitive resolver against wide df columns
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      val condOk: Column = r.conditionRaw match {
        ...
        case Some(c0) =>
          ...
          val rewritten = RuleDependency.rewriteCountFunctions(cleaned) // ✅ NEW
  ...
  // Build mapped columns for ONE base-table pass
...
// =================================================================================================
// AUTOMATION ENGINE (returns Map[DestTableName -> DataFrame])
// - Builds each destination by looping base source tables and unioning results (keeps duplicates)
// - Adds stable Id + _BaseSourceTable to prevent overwrite/merge behavior
// =================================================================================================
object MappingAutomation {

  def defaultTableLoader(spark: SparkSession): String => DataFrame = (tableName: String) => spark.table(tableName)
  ...
  // Add stable "Id" without dropping duplicates (preserves multiple rows per Crash_Id)
  private def addIdWithoutDroppingDuplicates(baseTable: String, graph: TableGraph, mappedDf: DataFrame): DataFrame = {
    ...
    val idParts: Seq[Column] =
      (basePk.map(c => coalesce(col(s"`$c`").cast("string"), lit(""))) :+ lit(baseTable) :+ col(rnColName).cast("string"))
  ...
  // MAIN: build all destination tables as Map
  def buildAllDestTablesAsMap(
    ...
    val outPairs = destSorted.map { dest =>
      ...
      // Build per base source and union (keeps duplicates)
      val perBase: Seq[DataFrame] = baseTablesOrdered.map { baseTable =>
        val (wide, baseCanon, _) = WideBuilder.buildWideDf(
          ...
          baseTable = baseTable,
          keepAllPk = true,
          countAggsForDest = neededCountAggsRaw // ✅ enables Unit.COUNT(Crash_Id)
        )
