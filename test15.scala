// =================================================================================================
// FINAL ONE-CELL SCRIPT (ADDS TYPE1 <VALUE> + <FORMAT> SUPPORT)
// -------------------------------------------------------------------------------------------------
// NEW FEATURE ✅ (Type 1 formatting):
//   If ConversionType = 1 AND SourceValue = <VALUE> AND DestValue = <FORMAT>
//   then output value is formatted using pattern from Condition column.
//   Example:
//     Source = Summary.Event_Date, SourceValue=<VALUE>, DestValue=<FORMAT>, Condition=yyyy-MM-dd
//     => CrashDate = date_format(cast(Event_Date as timestamp), "yyyy-MM-dd")
//
// IMPORTANT:
// - For <FORMAT> rules, "Condition" is treated as FORMAT PATTERN (NOT boolean predicate).
// - Works for Timestamp/Date columns and for string columns that Spark can cast to timestamp.
// =================================================================================================
...
// ------------------------------
// DestTableBuilder (Type2 NULL wildcard + Type1 <FORMAT> support)
// ------------------------------
object DestTableBuilder {
  ...
  private def buildExpr(
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      val isFmt = isFormatToken(raw) && r.conversionType == 1 && isValueToken(r.sourceValueRaw)

      // Condition:
      // - for <FORMAT> rules => Condition is format pattern, not boolean predicate => condOk = true
      // - otherwise parse boolean predicate normally
      val condOk: Column =
        if (isFmt) lit(true)
      ...
      // Destination value:
      // - if DestValue is <FORMAT> and SourceValue is <VALUE> (Type1):
      //     format source column using pattern from Condition
      // - else if DestValue is <VALUE>: pass-through from source column
      // - else literal
      val destVal: Column = {
        if (isFmt) {
          ...
          val src = resRule(srcCol)
          // cast to timestamp for date_format; works for TimestampType/DateType and parseable strings
          date_format(src.cast("timestamp"), fmt)
  ...
  // build mapped df for one destination using one baseTable (no cross enrich)
  def buildForDestTable(destTableName: String, baseTable: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): (DataFrame, Seq[String]) = {
    ...
    rulesByDestCol.toSeq.sortBy(_._1).foreach { case (destCol, allRulesSorted) =>
      ...
      // Text column: never applies <FORMAT> (description is literal)
