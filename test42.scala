...
object DestTableBuilder {
  ...
  private def buildExpr(
    ...
    def evalRule(r: MapRule): (Column, Column) = {
      ...
      val srcOk: Column = (r.sourceColumn, r.sourceValueRaw) match {
        ...
        case (Some(sc), None) =>
          ...
          if (hasCond) lit(true) else resRule(sc).isNull
      ...
      val ruleMatch = srcOk && condOk
      (ruleMatch, destVal)
    }

    // Normal rules: first match wins by priority (do not evaluate lower-priority rules).
    if (!whenOnlyIfAccNull) {
      val chainOpt = rules.foldLeft(Option.empty[Column]) { case (accOpt, r) =>
        val (ruleMatch, destVal) = evalRule(r)
        accOpt match {
          case None       => Some(when(ruleMatch, destVal))
          case Some(acc)  => Some(acc.when(ruleMatch, destVal))
        }
      }
      chainOpt.getOrElse(lit(null).cast("string"))
    } else {
      // Default rules: only apply when accumulator is still NULL.
      rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
        val (ruleMatch, destVal) = evalRule(r)
        val effectiveCond = ruleMatch && acc.isNull
  ...
  def buildForDestTable(
    ...
    rulesByDestCol.toSeq.sortBy(_._1).foreach { case (destCol, allRulesSorted) =>
      ...
      val textSelector: MapRule => Option[String] = r => r.destValueDescRaw.orElse(r.destValueRaw)
      val normalTxt = buildExpr(destTableName, destCol, normalRules, out, whenOnlyIfAccNull = false, textSelector, passThroughOnValueToken = false)
      val defTxt    = buildExpr(destTableName, destCol, defaultRules, out, whenOnlyIfAccNull = true,  textSelector, passThroughOnValueToken = false)
...
object MappingAutomation {
  ...
  def buildAllDestTablesAsMap(
    ...
    dedupeDestByPk: Set[String] = Set.empty,
    ...
    val outPairs: Seq[(String, DataFrame)] = destSorted.map { dest =>
      ...
      val deduped =
        if (!dedupeDestByPk.exists(_.equalsIgnoreCase(dest))) unioned
        else {
          val lowerToActual = unioned.columns.groupBy(_.toLowerCase).map { case (k, vs) => k -> vs.head }
          val allPkLogical = involvedSourceTables.flatMap(t => pkByTable.getOrElse(t, Nil)).distinct
          val allPkActual  = allPkLogical.flatMap(pk => lowerToActual.get(pk.toLowerCase)).distinct
          if (allPkActual.nonEmpty) unioned.dropDuplicates(allPkActual)
          else unioned
        }
