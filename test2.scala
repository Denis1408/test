// =================================================================================================
// ONE-CELL FINAL SCRIPT (UPDATED with <DEFAULT> fallback rules)
// -------------------------------------------------------------------------------------------------
// NEW LOGIC (your screenshot):
//   - If SourceColumnName is NULL AND SourceValue = "<DEFAULT>" (text token),
//     then this rule is a DEFAULT / FALLBACK for the destination column.
//   - It must be applied ONLY when no other (non-default) rule produced a value
//     (i.e. destination value is still NULL after evaluating normal rules).
//
// Example:
//   SourceColumnName = NULL
//   SourceValue      = <DEFAULT>
//   DestColumnName   = HitandRun
//   DestValue        = 0
//   DestValueDescription = "No"
//   Priority         = 999
//
// Result: if none of the other HitandRun rules matched, set HitandRun=0 and HitandRunText="No".
//
// ALSO INCLUDED (from previous requirements):
//   ✅ ConversionType=2: if SourceValue is "<NULL>" (text token), source value can be ANY (wildcard)
//   ✅ DestValueDescription -> new column DestColumnName + "Text"
//   ✅ Final output DF contains ONLY: base PK + mapped columns (+Text)
//   ✅ Token <VALUE> support (pass-through), Priority, Conditions parsing, auto wide join
// =================================================================================================
...
// ------------------------------
// DestTableBuilder: apply rules + Text columns + ONLY PK + mapped cols (+Text)
// Includes <DEFAULT> fallback rules
// ------------------------------
object DestTableBuilder {
  ...
  // Resolver: dot-safe + unqualified resolution + ambiguity resolved by SourceTableName if possible
  private def resolver(wideDf: DataFrame, defaultTable: Option[String]): String => Column = {
    (name0: String) => {
      ...
      if (wideDf.columns.contains(name)) litCol(name)
      else if (wideDf.columns.contains(tail)) litCol(tail)
      else {
  ...
  // Build mapping expr for a set of rules; whenOnlyIfAccNull:
  //   - false => normal behavior (apply rules by priority)
  //   - true  => fallback behavior (apply rule ONLY when accumulated is null)
  private def buildExpr(
    ...
    valueSelector: MapRule => Option[String],          // chooses DestValueRaw or DestValueDescRaw
    passThroughOnValueToken: Boolean                   // only for main value column
  ): Column = {
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      val srcOk: Column = (r.sourceColumn, r.sourceValueRaw) match {
        // DEFAULT rules have sourceColumn=None; srcOk=true (Condition may still exist)
        case (None, _) => lit(true)

        // <VALUE> => wildcard any
        case (Some(_), sv) if isValueToken(sv) => lit(true)

        // NEW: ConversionType=2: "<NULL>" means wildcard any
        case (Some(_), sv) if r.conversionType == 2 && isNullTextToken(sv) => lit(true)

        // SQL NULL => match nulls
      ...
      val destValStrOpt = valueSelector(r)

      val destVal: Column =
        if (passThroughOnValueToken && isValueToken(destValStrOpt)) {
          r.sourceColumn.map(resRule).getOrElse(lit(null))
        } else {
          destValStrOpt.map(lit).getOrElse(lit(null))
        }

      // DEFAULT behavior: only write if acc is still null
  ...
  // Main builder: returns ONLY base PK + mapped cols (+Text)
  def buildForDestTable(destTableName: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): DataFrame = {

    // Read mapping rows for this destination table (includes DestValueDescription)
    val rows = mappingDf
    ...
    // Create MapRules and group by DestColumnName
      ...
      rows
        .map { r =>
          MapRule(
            ...
            sourceColumn     = cleanOpt(r.getAs[Any]("SourceColumnName")), // NULL => None (your default rows)
    ...
    rulesByDestCol.toSeq.sortBy(_._1).foreach { case (destCol, allRulesSorted) =>
      ...
      //   - SourceColumnName is NULL (=> None)
      //   - SourceValue is "<DEFAULT>"
      // Apply them only if normal rules produced null
      ...
      val normalValExpr =
        buildExpr(
          ...
          rules         = normalRules,
          outDf         = out,
          whenOnlyIfAccNull = false,
          valueSelector = _.destValueRaw,
          passThroughOnValueToken = true
        )

      // Default value expression (only if still null)
      val defaultValExpr =
        buildExpr(
          destTableName = destTableName,
          destCol       = destCol,
          rules         = defaultRules,
          outDf         = out,
          whenOnlyIfAccNull = true, // apply only when accumulator is null
          valueSelector = _.destValueRaw,
          passThroughOnValueToken = true
        )

      val finalValExpr = coalesce(normalValExpr, defaultValExpr)

      // Normal text expression
      val normalTextExpr =
        buildExpr(
          destTableName = destTableName,
          destCol       = destCol,
          ...
          outDf         = out,
          whenOnlyIfAccNull = false,
          valueSelector = _.destValueDescRaw,
          passThroughOnValueToken = false
        )
      ...
      val defaultTextExpr =
        buildExpr(
          ...
          rules         = defaultRules,
          outDf         = out,
          whenOnlyIfAccNull = true,
          valueSelector = _.destValueDescRaw,
          passThroughOnValueToken = false
        )

      val finalTextExpr = coalesce(normalTextExpr, defaultTextExpr)

      // Add columns; if already exists, keep existing when mapping doesn't hit
      out =
        if (out.columns.contains(destCol)) out.withColumn(destCol, coalesce(finalValExpr, col(s"`$destCol`")))
