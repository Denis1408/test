...
object DestTableBuilder {
  ...
  private def buildExpr(
    ...
    val noSrcKey = "__NO_SRC__"
    val (finalVal, _) = rules.foldLeft((lit(null).cast("string"), Map.empty[String, Column])) { case ((acc, matchedBySrc), r) =>
      ...
      val key = r.sourceColumn.getOrElse(noSrcKey)
      val prevMatched = matchedBySrc.getOrElse(key, lit(false))
      val effectiveCond =
        if (whenOnlyIfAccNull) ruleMatch && acc.isNull
        else ruleMatch && !prevMatched

      val nextAcc = when(effectiveCond, destVal).otherwise(acc)
      val nextMatchedBySrc = matchedBySrc.updated(key, prevMatched || ruleMatch)
      (nextAcc, nextMatchedBySrc)
    }
    finalVal
  }
