...
//    - normalizes quotes
//    - removes trailing ';' and dangling AND/OR
...
object ConditionParser {
  ...
  private def tokenize(input0: String): Vector[Tok] = {
    ...
    def isIdPart(ch: Char) = ch.isLetterOrDigit || ch == '_' || ch == '.' // allow dotted names
...
// =================================================================================================
// 3) RULE DEPENDENCY EXTRACTION (UPDATED)
//    Now returns BOTH:
//      - needed columns per table from mapping + "Table.Col" in conditions
//      - needed correlated COUNT aggregates from patterns "Table.Count(Col)"
// =================================================================================================
object RuleDependency {

  // Detect "Table.Column" references
  private val tableColRx: Regex =
    """([A-Za-z_][A-Za-z0-9_]*)\s*\.\s*([A-Za-z_][A-Za-z0-9_]*)""".r

  // Detect "Table.Count(Column)" references (case-insensitive Count)
  ...
  def requiredDepsForDest(mappingDf: DataFrame, destTable: String): (Map[String, Set[String]], Map[String, Set[String]]) = {
    ...
    val countAcc = scala.collection.mutable.Map.empty[String, scala.collection.mutable.Set[String]]

    def addCol(t: String, c: String): Unit  = colsAcc.getOrElseUpdate(t, scala.collection.mutable.Set.empty) += c
    def addCnt(t: String, c: String): Unit  = countAcc.getOrElseUpdate(t, scala.collection.mutable.Set.empty) += c
    ...
    rows.foreach { r =>
      ...
      // from explicit columns in mapping
      for (t <- st; c <- sc) addCol(t, c)

      // from Table.Column occurrences in condition
      tableColRx.findAllMatchIn(cond).foreach { m =>
        val t = m.group(1)
        val c = m.group(2)
        // IMPORTANT: this regex also matches "Unit.Count" as Table.Column.
        // We do NOT want to treat "Count" as a real column dependency.
        // So skip when column part is Count (case-insensitive).
        if (!c.equalsIgnoreCase("Count")) addCol(t, c)
      }

      // from Table.Count(Column) occurrences in condition
      tableCountRx.findAllMatchIn(cond).foreach { m =>
        addCnt(m.group(1), m.group(2))
      }
    }

    (colsAcc.view.mapValues(_.toSet).toMap, countAcc.view.mapValues(_.toSet).toMap)
  }

  // Rewrite "Table.Count(Col)" => "__agg__Table__count__Col"
  // This makes ConditionParser treat it like a normal column reference.
...
// =================================================================================================
// 4) WIDE BUILDER (UPDATED)
//    Builds wide DF and ALSO attaches correlated COUNT aggregates requested by RuleDependency.
// =================================================================================================
object WideBuilder {

  // Canonicalize table name in a case-insensitive way using graph tables
  ...
  // Select keys and required cols; alias keys optionally; alias data as "Table.Col"
  ...
  // Determine correct keys for direction along edge
  ...
  // Resolve a column name in wide DF case-insensitively:
  // - try full match
  // - try tail match
  // - try unique dotted match (endsWith ".tail")
  ...
  // Attach correlated count aggregates into wide DF:
  // For each (table, col):
  //   aggDf = tableDf.groupBy(col).count() => __agg__Table__count__Col
  // Then join wide on wide.<col> == aggDf.<col>.
  private def attachCountAggs(
    graph: TableGraph,
    wideDf: DataFrame,
    countAggsRaw: Map[String, Set[String]]
  ): DataFrame = {
    if (countAggsRaw.isEmpty) return wideDf

    // Canonicalize table names as they appear in graph
    ...
    countAggs.foldLeft(wideDf) { case (curWide, (tableName, colsToCount)) =>
      ...
      colsToCount.foldLeft(curWide) { case (wide2, colToCount) =>
        // Find actual column name in SOURCE table (case-insensitive)
        val srcActualOpt = node.df.columns.find(_.equalsIgnoreCase(colToCount))
        if (srcActualOpt.isEmpty) {
          // If source column doesn't exist, fail fast
        ...
        // Build aggregate: count rows per srcActual
        val aggColName = s"__agg__${tableName}__count__${srcActual}" // (keeps actual case from source col)
        val aggDf = node.df
          .groupBy(col(s"`$srcActual`").as(srcActual)) // keep join key column
          .agg(count(lit(1)).cast("long").as(aggColName))

        // Join wide on "current row" key == aggregate key
        // We resolve the join key column in wide case-insensitively by name 'srcActual'
        val wideJoinKey = resolveWideColCaseInsensitive(wide2, srcActual)

        // Note: aggDf join key column is named srcActual
        val joined = wide2.join(aggDf, wideJoinKey === aggDf.col(s"`$srcActual`"), "left")
          .drop(aggDf.col(s"`$srcActual`")) // remove duplicate join key from aggDf side
  ...
  // Build wide DF and then attach correlated counts
  def buildWideDf(
    graph: TableGraph,
    mappingDf: DataFrame,
    destTable: String,
    ...
    // Select+alias function for a joined table (prefix PK, dotted data cols)
    ...
    // Join every needed table via path from base
    ...
    // NEW: attach correlated count aggregates
...
// =================================================================================================
// 5) DEST TABLE BUILDER (UPDATED TO REWRITE Count() IN CONDITIONS)
// =================================================================================================
object DestTableBuilder {
  ...
  // Try to parse string date/time into timestamp
  ...
  // Case-insensitive column resolver against wide DF
  ...
  // Build a destination expression for one dest column from prioritized rules
  private def buildExpr(
    ...
    rules.foldLeft(lit(null).cast("string")) { case (acc, r) =>
      ...
      // Type1 special formatting (SourceValue=<VALUE> and DestValue=<FORMAT>)
      val fmtRule: Boolean =
      ...
      val srcOk: Column = (r.sourceColumn, r.sourceValueRaw) match {
        ...
        // Type2 wildcard: SourceValue <NULL> or SourceValue NULL => any value, condition decides
      ...
      // Condition logic:
      // - If fmtRule: condition is format string (not boolean)
      // - Else: sanitize, REWRITE Count(), then parse
      val condOk: Column =
        if (fmtRule) lit(true)
      ...
      // Destination value for this rule
      val destVal: Column =
        if (fmtRule && isFormatToken(raw)) {
          val fmt = r.conditionRaw.map(_.trim).filter(_.nonEmpty)
            .getOrElse(throw new IllegalArgumentException(s"Missing format pattern in Condition for Dest=$destTableName.$destCol"))
          val srcCol = r.sourceColumn.getOrElse(throw new IllegalArgumentException(s"Missing SourceColumnName for Dest=$destTableName.$destCol"))
  ...
  // Build mapped DF for one destination + one base table pass
  def buildForDestTable(destTableName: String, baseTable: String, basePk: Seq[String], wideDf: DataFrame, mappingDf: DataFrame): (DataFrame, Seq[String]) = {
    ...
    // Pull rules for this destination; keep only those for this base source table (plus defaults)
    val rows = mappingDf
    ...
    // Normalize rules and group by destination column, ordering by Priority
    ...
    // Apply mapping for each destination column
    rulesByDestCol.toSeq.sortBy(_._1).foreach { case (destCol, allRulesSorted) =>
      val destTextCol = s"${destCol}Text"

      // <DEFAULT> rules: SourceColumnName is null and SourceValue is <DEFAULT>
    ...
    // Only output: base PKs + mapped columns (+Text)
...
// =================================================================================================
// 6) AUTOMATION ENGINE
//    - Builds Map[DestTableName -> DataFrame]
//    - Supports excludeDestIdFor, renameRulesByDest
//    - Keeps ALL rows from multiple source tables via unionByName (no overwrites)
// =================================================================================================
object MappingAutomation {
  ...
  // Normalize joinEdges input to JoinEdge objects (supports tuples as well)
  ...
  // Load only needed source tables for this destination into the graph
  ...
  // Add stable row Id without dropping duplicates; also add marker _BaseSourceTable
  ...
  // Add <DestName>Id at end, per Crash_Id (optional exclusion)
  ...
  // =================================================================================================
  // MAIN: Build all destination tables and return Map[DestTableName -> DataFrame]
  // =================================================================================================
  def buildAllDestTablesAsMap(
    ...
    // Canonicalize join edges to sourceUniverse
    ...
    val outPairs = destSorted.map { dest =>
      ...
      // Extract dependencies for this dest table:
      // - needed columns per table
      // - needed Count() aggregates per table
      val (neededColsRaw, neededCountAggsRaw) = RuleDependency.requiredDepsForDest(mappingDf, dest)

      // Also include SourceTableName from mapping rows (base candidates)
      ...
      // Build graph only for needed tables (load dataframes here)
      val graph = buildGraphForTables(tableLoader, pkByTable, joinEdges, involvedSourceTables.toSet)

      // Order base tables by sourceTablePriority
      ...
      // For each base source table build a mapped DF, then union them (keeping ALL rows)
      ...
      // Apply final renames (if provided for this dest)
